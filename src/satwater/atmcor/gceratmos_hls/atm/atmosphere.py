    ...
    def run(self):
        ...
        if self.parameters.satellite == 'MSI_S2':
            if self.parameters.type == 'S2A':
                rsr = pd.read_csv(r'data\RSR\rsr_S2A_MSI.txt', sep='\t', skiprows=5, names=['wavelength', 'rsr', 'band', 'id'])
                bands = self.rsr_interp(rsr, 13, [0.412, 2.321])
            else:
                rsr = pd.read_csv(r'data\RSR\rsr_S2B_MSI.txt', sep='\t', skiprows=5, names=['wavelength', 'rsr', 'band', 'id'])
                bands = self.rsr_interp(rsr, 13, [0.412, 2.321])
        elif self.parameters.satellite == 'OLI_L8/9':
            if self.parameters.type == 'LC08':
                rsr = pd.read_csv(r'data\RSR\rsr_L8_OLI.txt', sep='\t', skiprows=5, names=['wavelength', 'rsr', 'band', 'id'])
                bands = self.rsr_interp(rsr, 8, [0.427, 2.356])
            else:
                rsr = pd.read_csv(r'data\RSR\rsr_L9_OLI2.txt', sep='\t', skiprows=5, names=['wavelength', 'rsr', 'band', 'id'])
                bands = self.rsr_interp(rsr, 8, [0.427, 2.356])
        else:
        ...
        for i in range(x, len(bands) + x):
            ...
            s.run()
            # Output:
            self.values[i - x] = {'view_zn': self.parameters.geometry[i - x]['view_zn'], 'view_az': self.parameters.geometry[i - x]['view_az'], 'solar_zn': self.parameters.geometry[i - x]['solar_zn'], 'solar_az': self.parameters.geometry[i - x]['solar_az'],
                                  'tg_OG_co': float(s.outputs.transmittance_co.total), 'tg_OG_c02': float(s.outputs.transmittance_co2.total), 'tg_OG_o2': float(s.outputs.transmittance_oxygen.total),
